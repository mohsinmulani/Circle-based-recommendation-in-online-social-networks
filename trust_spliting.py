...
ofd=open("Sample_output_Trust_Spliting_5_fold","w")
...
for loop_counter in range(1,6):
	...
	for user in List_Users:
	    Dict_User_Category[user]=set()

	# print Training_df.head(n=60)
	List_Category_row=[]
	for i in range(len(Training_df)):
	    category= Training_df.iloc[i]['Category']
	    user=Training_df.iloc[i]['User_id']
	    List_Category_row.append(category)
	    Dict_User_Category[user].add(category)
	    
	for i in range(users):
	    user=User_Trust_Network_df['User_id'][i]
	    if user in Dict_User_Category:
	        Cat_list1=Dict_User_Category[user]
	        row_index=List_Users.index(user)
	        if len(Cat_list1)>0:
	            trustee=User_Trust_Network_df['Trustee'][i]
	            for trustee_id in trustee:
	                if trustee_id in Dict_User_Category:
	                    Cat_list2=Dict_User_Category[trustee_id]
	                    Same_Category = Cat_list1.intersection(Cat_list2)
	                    Same_Category_WT=0.0
	                    if len(Same_Category)>0:

	                        for cat in Same_Category:
	                        	if cat not in cat_list:
	                        		continue
	                        	Same_Category_WT+=Category_Circle[cat][trustee_id]
	                        if Same_Category_WT!=0.0:
	                            for cat in Same_Category:
	                            	if cat not in cat_list:
	                            		continue
