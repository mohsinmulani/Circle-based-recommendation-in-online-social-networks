...
Distribution_vector = defaultdict(lambda : defaultdict(float))
Followers={}
...
ofd=open("Sample_output_Expertise_based_varient_b_5_fold","w")
...
for loop_counter in range(1,6):
	...
	#Filling Matrix S as Category_Circle (Varient b)
	for user in List_Users:
	    total_row_WT=0.0
	    for Category in Category_Circle.keys():
	        total_row_WT+=Category_Circle[Category][user]
	    if total_row_WT > 0.0:
	        for Category in Category_Circle.keys():    
	            Distribution_vector[Category][user]=Category_Circle[Category][user]/total_row_WT

	# for user in List_Users:
	#     print "User_id :"+str(user)
	#     for Category in Category_Circle.keys():
	#         print Category,Distribution_vector[Category][user],
	#     print "\n"

	for i in range(users):
	    user=User_Trust_Network_df['User_id'][i]
	    trustee=User_Trust_Network_df['Trustee'][i]
	    for trustee_id in trustee:
	        if trustee_id not in Followers:
	            Followers[trustee_id]=list()
	        Followers[trustee_id].append(user)
	...
	for Category in Category_Circle.keys():
		...
		for i in range(users):
			user = User_Trust_Network_df['User_id'][i]
			if user in category_user_list and user in List_Users:
			    trustee = User_Trust_Network_df['Trustee'][i]
			    ...
			    for trustee_user in Same_Category_trustee:
			        Voting_value=0.0
			        for follower in Followers[trustee_user]:
			            Voting_value+=Distribution_vector[Category][follower]
			        row_index = List_Users.index(user)
			        col_index = List_Users.index(trustee_user)
			        S[row_index][col_index] = Category_Circle[Category][trustee_user]*Voting_value
			        total_row_WT+= S[row_index][col_index]
			    if total_row_WT != 0.0:
			        for trustee_user in Same_Category_trustee:
			            row_index = List_Users.index(user)
			            col_index = List_Users.index(trustee_user)
			            S[row_index][col_index] /= total_row_WT
