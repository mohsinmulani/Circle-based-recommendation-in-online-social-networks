...
ofd=open("Sample_output_Expertise_based_Varient_a_5_fold","w")
	...
	for Category in Category_Circle.keys():
		...
		for i in range(users):
			...
			if user in category_user_list and user in List_Users:
				...
				total_row_WT = sum(Category_Circle[Category][trustee_user] for trustee_user in Same_Category_trustee)
				total_row_WT = float(total_row_WT)
			if total_row_WT != 0.0:
				for trustee_user in Same_Category_trustee:
					row_index = List_Users.index(user)
					col_index = List_Users.index(trustee_user)
					S[row_index][col_index] = Category_Circle[Category][trustee_user]/total_row_WT
